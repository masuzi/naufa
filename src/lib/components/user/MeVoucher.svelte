<script lang="ts">
  ...
  import { getVoucher } from './graphql'
  ...
  const options = {
    ...
    body: JSON.stringify(getVoucher(handphone))
  }
  let vouchers = (async () => {
...
<div in:fade out:fly={{ x: 400 }} class="pt-4 px-4 w-full flex flex-col">
  ...
  <div class="rounded-md bg-white px-3 my-4">
    <ul class="my-4 space-y-3">
      {#await vouchers then v}
        {#each v.data.getVoucher.vouchers as voucher}
          {#if voucher.status === '01'}
            <li>
              <div class="flex justify-between mb-1 p-1">
                <div class="text-gray-600">No: {voucher.voucher_num}</div>
                <!-- <div class="text-gray-600">Berlaku hingga: {voucher.createdAt}</div> -->
              </div>
            ...
            <li class="bg-green-100 border border-green-300">
              <div class="flex justify-between mb-3 p-1">
                ...
                <div class="text-gray-600">
                  Tgl. Beli: {new Date(voucher.createdAt / 1).getDate()}-{new Date(
                    voucher.createdAt / 1
                  ).getMonth() + 1}-{new Date(voucher.createdAt / 1).getFullYear()}
                </div>
              </div>
              <div
                class="flex items-center p-1  text-gray-900 bg-green-50 hover:bg-gray-100 group hover:shadow "
              >
                <span class="flex-1 ml-1 whitespace-nowrap w-1/4 text-gray-500">Name</span
                >
                <span
                  class="w-1/4 inline-flex items-center justify-center px-2 py-0.5 ml-3 text-xs  text-gray-500 "
                  >Kode</span
                >
                <span
                  class="w-1/4 inline-flex items-center justify-center px-2 py-0.5 ml-3 text-xs  text-gray-500 "
              ...
              <a
                href="#"
                class="flex items-center p-1  text-gray-900  rounded-lg hover:bg-gray-100 group hover:shadow "
              >
                <!-- <span class="flex-1 ml-1 whitespace-nowrap w-1/4"
                  >{voucher.item.name}</span
                > -->
